...
func (s *subscriber) ping() {
	...
	for {
		...
		case <-ticker.C:
			...
			if err != nil {
				logger.Errorf("subscriber error writing ping message: %v", err)
				return
			}
		case <-s.exit:
			return
...
func (s *subscriber) run() {
	...
	for {
		...
		if err := json.Unmarshal(message, &msg); err != nil {
			// do what?
			logger.Errorf("subscriber error unmarshaling message: %v", err)
			continue
		}
		p := &publication{topic: s.topic, message: msg}
		p.err = s.handler(p)
		if p.err != nil {
			logger.Errorf("handler execution error: %v", err)
		}
	}
}
